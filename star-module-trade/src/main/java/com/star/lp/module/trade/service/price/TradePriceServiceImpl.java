...
import com.star.lp.module.member.api.level.dto.MemberLevelRespDTO;
...
import com.star.lp.module.promotion.api.discount.DiscountActivityApi;
import com.star.lp.module.promotion.api.discount.dto.DiscountProductRespDTO;
import com.star.lp.module.promotion.api.reward.RewardActivityApi;
import com.star.lp.module.promotion.api.reward.dto.RewardActivityMatchRespDTO;
import com.star.lp.module.promotion.enums.common.PromotionTypeEnum;
...
import com.star.lp.module.trade.service.price.calculator.TradeDiscountActivityPriceCalculator;
import com.star.lp.module.trade.service.price.calculator.TradePriceCalculator;
...
import com.star.lp.module.trade.service.settingoption.PrintPriceService;
...
import static com.star.lp.module.product.enums.ErrorCodeConstants.SKU_STOCK_NOT_ENOUGH;
import static com.star.lp.module.trade.enums.ErrorCodeConstants.PRICE_CALCULATE_PAY_PRICE_ILLEGAL;
import static com.star.lp.module.trade.enums.ErrorCodeConstants.PRICE_CALCULATE_PAY_PRICE_ILLEGAL_CHECK_PRICE;
...
public class TradePriceServiceImpl implements TradePriceService {
    ...
    public TradePriceCalculatePrintRespBO calculateOrderPrintPrice(TradePriceCalculatePrintReqBO calculateReqBO) {
        ...
        calculateReqBO.getItems().forEach(item -> {
            // 调用打印价格服务计算每个项目的单价
            Integer checkPrice = printPriceService.calculateTotalPrice(item.getSelectedOptions(),item.getTotalPages(), item.getCopyCount());
            System.out.print(checkPrice);
            System.out.print(item.getTotalPrice());

            if (!Objects.equals(item.getTotalPrice(), checkPrice)) {
                throw exception(PRICE_CALCULATE_PAY_PRICE_ILLEGAL_CHECK_PRICE);
            }
        });
        ...
        // 遍历所有价格计算器并执行计算
//        priceCalculatorsPrint.forEach(calculator -> calculator.calculate(calculateReqBO, calculateRespBO));
