...
import com.star.lp.module.trade.controller.admin.delivery.vo.logistics.DeliveryPickUpStoreLogisticsConfigRespVO;
...
public class DeliveryPickUpStoreController {
    ...
    public CommonResult<DeliveryPickUpStoreRespVO> getDeliveryPickUpStore(@RequestParam("id") Long id) {
        ...
        // 获取物流配置列表
        List<DeliveryPickUpStoreLogisticsConfigRespVO> logisticsConfigs = logisticsConfigService.getLogisticsConfigListByStoreId(id).stream()
                .map(config -> BeanUtils.toBean(config, DeliveryPickUpStoreLogisticsConfigRespVO.class))
                .toList();
        ...
        return success(BeanUtils.toBean(deliveryPickUpStore, DeliveryPickUpStoreRespVO.class)
                ...
                .setLogisticsConfigs(logisticsConfigs)
                );
    }
    ...
    public CommonResult<PageResult<DeliveryPickUpStoreRespVO>> getDeliveryPickUpStorePage(@Valid DeliveryPickUpStorePageReqVO pageVO) {
        ...
        // 转换为响应VO
        PageResult<DeliveryPickUpStoreRespVO> respPageResult = DeliveryPickUpStoreConvert.INSTANCE.convertPage(pageResult);
        
        // 为每个门店添加物流配置数据
        respPageResult.getList().forEach(respVO -> {
            List<DeliveryPickUpStoreLogisticsConfigRespVO> logisticsConfigs = logisticsConfigService.getLogisticsConfigListByStoreId(respVO.getId()).stream()
                    .map(config -> BeanUtils.toBean(config, DeliveryPickUpStoreLogisticsConfigRespVO.class))
                    .toList();
            respVO.setLogisticsConfigs(logisticsConfigs);
        });
    ...
    @GetMapping("/logistics-config/list")
    @Operation(summary = "获得自提门店物流配置列表")
    @Parameter(name = "storeId", description = "门店ID", required = true, example = "1024")
    @PreAuthorize("@ss.hasPermission('trade:delivery:pick-up-store:query')")
    public CommonResult<List<DeliveryPickUpStoreLogisticsConfigRespVO>> getLogisticsConfigList(@RequestParam("storeId") Long storeId) {
        return success(logisticsConfigService.getLogisticsConfigListByStoreId(storeId).stream()
                .map(config -> BeanUtils.toBean(config, DeliveryPickUpStoreLogisticsConfigRespVO.class))
                .toList());
    }
