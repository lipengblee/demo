...
import com.star.lp.module.trade.controller.admin.order.vo.TradeOrderSummaryRespVO;
...
import com.star.lp.module.trade.framework.delivery.core.client.ExpressClientFactory;
import com.star.lp.module.trade.framework.delivery.core.client.dto.ExpressTrackQueryReqDTO;
...
import static com.star.lp.module.trade.enums.ErrorCodeConstants.EXPRESS_NOT_EXISTS;
import static com.star.lp.module.trade.enums.ErrorCodeConstants.ORDER_NOT_FOUND;
...
public class TradeOrderQueryServiceImpl implements TradeOrderQueryService {
    ...
    @Override
    public PageResult<TradeOrderDO> getAppointStoreOrderPage(TradeOrderAppointPageReqVO reqVO) {
        // 根据用户查询条件构建用户编号列表
        Set<Long> userIds = buildQueryConditionUserIds(reqVO);
        if (userIds == null) { // 没查询到用户，说明肯定也没他的订单
            return PageResult.empty();
        ...
        // 分页查询
        return tradeOrderMapper.selectAppointStorePage(reqVO, userIds);
    }
    ...
    private Set<Long> buildQueryConditionUserIds(TradeOrderAppointPageReqVO reqVO) {
        // 获得 userId 相关的查询
        Set<Long> userIds = new HashSet<>();
        if (StrUtil.isNotEmpty(reqVO.getUserMobile())) {
            MemberUserRespDTO user = memberUserApi.getUserByMobile(reqVO.getUserMobile());
    ...
    @Override
    public TradeOrderSummaryRespVO getAppointStoreOrderSummary(TradeOrderAppointPageReqVO reqVO) {
        // 根据用户查询条件构建用户编号列表
        Set<Long> userIds = buildQueryConditionUserIds(reqVO);
        if (userIds == null) { // 没查询到用户，说明肯定也没他的订单
            return new TradeOrderSummaryRespVO();
        ...
        // 查询每个售后状态对应的数量、金额
        List<Map<String, Object>> list = tradeOrderMapper.selectAppointStoreOrderSummaryGroupByRefundStatus(reqVO, userIds);
    ...
    @Override
    public List<TradeOrderDO> getOrdersByLogisticsNo(String logisticsNo) {
        return tradeOrderMapper.selectByLogisticsNo(logisticsNo);
    }

    @Override
    public TradeOrderDO getLatestOrderByLogisticsNo(String logisticsNo) {
        return tradeOrderMapper.selectLatestByLogisticsNo(logisticsNo);
    }

    @Override
    public TradeOrderDO getOrderByNo(String orderNo) {
        return tradeOrderMapper.selectByNo(orderNo);
    }
    ...
    @Override
    public List<ProductPrintDocumentSpuDO> selectBySpuIds(Collection<Long> orderIds) {
        return productPrintDocumentSpuMapper.selectBySpuIds(new ArrayList<>(orderIds));
    }
